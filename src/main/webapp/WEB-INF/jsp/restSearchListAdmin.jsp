					    ...
					    <form method="post"  action="/goRestSearchAdmin.do">
					        <select name="field">			    
							    <c:set var = "tag" value = "${field }" />
									<option value="r_num"
										<c:if test="${tag == 'r_num'}">selected</c:if>>최신순</option>			
									<option value="r_name"
										<c:if test="${tag == 'r_name'}">selected</c:if>>상호명순</option>	
									<option value="r_food"
										<c:if test="${tag == 'r_food'}">selected</c:if>>주된음식순</option>			
									<option value="r_category"
										<c:if test="${tag == 'r_category'}">selected</c:if>>업태명순</option>			
							  </select>
