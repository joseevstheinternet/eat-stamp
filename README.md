...
`Layout` `메인화면` `키워드 추출` `게시글` `댓글` `신고` `마이페이지` `식당 예약`
`관리자 메인` `관리자 회원 관리` `관리자 사장님 관리` `관리자 게시글 관리`
`사장님 메인` `사장님 예약 관리` `사장님 식당 관리` `사장님 리뷰 관리`

- **최은지** 


`회원가입/탈퇴` `로그인/로그아웃` `식당상세` `검색` `찜한 식당` `세션 타임아웃`
`관리자 로그인` `관리자 식당 관리` `관리자 식당 등록` `관리자 신고 관리`
